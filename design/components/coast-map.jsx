// California coast map — travel-poster hero illustration
// Stylized state silhouette with all beach stations as warm dots, sized by risk.
// Hand-drawn coastline path approximating CA shape, lat/lon projected to viewport.

// Coast path — stylized California silhouette (W coast detailed, E coast simplified)
// viewBox 0 0 600 800. North is up. Coastline runs left side from ~y=20 to y=780.
const CA_PATH = `
  M 245 18
  L 460 22
  L 470 60
...
// Project lat/lon (CA bbox) to viewBox 600x800 — the coast roughly traces the LEFT edge.
const LAT_MIN = 32.4, LAT_MAX = 41.95;
const LON_MIN = -124.5, LON_MAX = -114.0;  // include inland for east-edge rectangle
function project(lat, lon) {
  // Map lat (y) — north (high lat) to top (low y)
  const y = 22 + (1 - (lat - LAT_MIN) / (LAT_MAX - LAT_MIN)) * 758;
  // Coastline x at that y, very roughly — interpolate hand-tuned anchors
  // (lat → x_coast on the silhouette)
  const anchors = [
    [41.9, 65], [41.0, 55], [40.0, 60], [39.0, 75], [38.5, 95],   // NorCal
    [38.0, 88], [37.7, 100], [37.0, 130], [36.5, 145], [36.0, 165], // Bay/Monterey
    [35.5, 195], [35.0, 220], [34.5, 245], [34.4, 270], [34.0, 305], // Central
    [33.7, 340], [33.4, 380], [33.0, 415], [32.8, 440], [32.4, 460],  // SoCal
  ];
  // Find bracketing anchors by lat
  let xCoast = 60;
  ...
  // Place dot just inside the coast (a few px right of coastline)
  const x = xCoast + 6 + ((lon + 122) * -8); // slight lon offset inland
  return { x, y, xCoast };
}

function CaliforniaPosterMap({ band = 'Moderate', activeBeach, onSelect, beaches, height = 640 }) {
  // Group beaches by region
  const grouped = React.useMemo(() => {
    const g = { NorCal: [], Central: [], SoCal: [] };
    beaches.forEach(b => { (g[b.region] || (g[b.region] = [])).push(b); });
    return g;
  }, [beaches]);
  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 600 800" preserveAspectRatio="xMidYMid meet"
        ...
        {/* Latitude graticule — thin dashed horizontals */}
        {[34, 36, 38, 40].map(lat => {
          const y = 22 + (1 - (lat - LAT_MIN) / (LAT_MAX - LAT_MIN)) * 758;
          return (
            <g key={lat}>
              ...
              <text x="8" y={y - 4} fill="#5e6b73" fontSize="9"
                fontFamily="var(--sl-mono)" letterSpacing="0.1em" opacity="0.7">
                {lat}°N
              </text>
            </g>
          );
        ...
        {/* CA landmass — soft fill + topo dot pattern */}
        <path d={CA_PATH} fill="url(#land)"/>
        <path d={CA_PATH} fill="url(#topo)" opacity="0.6"/>

        {/* Coastline hairline (just the W edge) */}
        <path d={`
        ...
        {/* SUN — travel poster signature, upper right over ocean */}
        ...
        {/* Title text inscribed on land — like an old map */}
        <text x="380" y="160" textAnchor="middle"
          fontFamily="var(--sl-display)" fontSize="38" fontWeight="450"
          fill="#8a6e2e" opacity="0.22" letterSpacing="0.18em">
          CALIFORNIA
        </text>
        <text x="380" y="190" textAnchor="middle"
          fontFamily="var(--sl-mono)" fontSize="9.5"
          fill="#8a6e2e" opacity="0.42" letterSpacing="0.32em">
          PACIFIC SHORELINE
        </text>

        {/* Region labels along the coast */}
        {[
          ...
          <text key={r.label} x={r.x} y={r.y}
            fontFamily="var(--sl-mono)" fontSize="9.5" fill="var(--sl-muted)"
            letterSpacing="0.28em" opacity="0.7">
            {r.label}
          </text>
        ))}

        {/* Beach stations — dots colored by today's risk */}
        {beaches.map(b => {
          const { x, y } = project(b.lat, b.lon);
          const isActive = activeBeach && activeBeach.id === b.id;
          const tok = RISK_TOKEN[b.risk];
        ...
        {/* Compass rose — bottom right corner over ocean */}
        <g transform="translate(540 720)">
          ...
          <text x="0" y="-26" textAnchor="middle" fontFamily="var(--sl-mono)" fontSize="9"
        ...
        {activeBeach && (() => {
          ...
          // Place callout to the right of the dot
          const cx = Math.min(p.x + 30, 420);
          const cy = Math.max(40, Math.min(p.y - 20, 720));
          const tok = RISK_TOKEN[activeBeach.risk];
          return (
            <g>
              ...
              <foreignObject x={cx} y={cy} width="200" height="86">
                <div xmlns="http://www.w3.org/1999/xhtml" style={{
                  ...
                  fontFamily: 'var(--sl-text)',
                  boxShadow: '0 4px 12px rgba(11,66,102,0.10)',
                }}>
                  <div className="sl-label" style={{ color: 'var(--sl-muted)', fontSize: 9 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                    ...
                    <span className="sl-mono" style={{ fontSize: 10, color: 'var(--sl-muted)' }}>
...
Object.assign(window, { CaliforniaPosterMap });
