...
careful to keep. Measured on the shipped frame with the source CSVs held fixed:
**1,009 observations destroyed, 120 of them exceedances, 51 beach-days flipped
1->0, 543 values moved (542 down, 0 up)** — every flip in the false-negative
direction.
...
def test_additive_source_replicates_survive() -> None:
    """1,206 SafeToSwim groups on the shipped frame are same-time replicates.
...
def test_exceedance_at_a_shared_timestamp_is_never_dropped() -> None:
    """The regression that mattered: 120 exceedances per run, always 1->0."""
    merged = _frame(
...
def test_repair_corrects_stored_culture_rows_reporting_copies() -> None:
    """The three Mendocino rows are months old; only a repair pass reaches them."""
    stored = _frame(
        _row(
            beach_id="ca290892-mendocino-hare-creek-hare-creek",
            method="Enterolert",
            units="Copies/100ml",
            value=20.0,
        ),
    ...
    out = repair_observation_units(stored, 104.0)
    assert out.iloc[0]["units"] == "MPN/100ml"
    # The genuine ddPCR row keeps its units and its 1413-based verdict.
    assert out.iloc[1]["units"] == "Copies/100ml"
    assert bool(out.iloc[1]["exceeds_stv"])


def test_repair_reverses_the_silent_false_negative() -> None:
    """500 MPN judged against 1413 reads clean; against 104 it exceeds."""
    stored = _frame(
        _row(method="Enterolert", units="Copies/100ml", value=500.0, exceeds_stv=False)
    )
    out = repair_observation_units(stored, 104.0)
    assert out.iloc[0]["units"] == "MPN/100ml"
    assert bool(out.iloc[0]["exceeds_stv"]) is True


def test_repair_is_a_noop_when_nothing_is_mislabelled() -> None:
    stored = _frame(_row(), _row(method="ddPCR", units="Copies/100ml", value=5000.0))
    out = repair_observation_units(stored, 104.0)
    pd.testing.assert_frame_equal(out, stored)


@pytest.mark.parametrize("bad_value", [None, float("nan")])
def test_repair_tolerates_missing_units(bad_value: object) -> None:
    stored = _frame(_row(units=bad_value))
    repair_observation_units(stored, 104.0)  # must not raise
